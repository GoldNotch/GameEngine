...
MeshRenderer::MeshRenderer(DrawTool_SDL & drawTool)
...
void MeshRenderer::RenderCache(const RenderTarget & renderTarget)
{
  auto renderPass = renderTarget.CreateSubPass();
  ...
  for (auto && [path, drawData] : m_gpuCache)
  {
    ...
    SDL_BindGPUVertexBuffers(renderPass.get(), 0, &vertexBufferBinding,
                             1); // bind one buffer starting from slot 0
    SDL_BindGPUIndexBuffer(renderPass.get(), &indexBufferBinding, SDL_GPU_INDEXELEMENTSIZE_32BIT);

    for (auto && cmd : drawData.commands)
    {
      SDL_DrawGPUIndexedPrimitives(renderPass.get(), static_cast<uint32_t>(cmd.indicesCount), 1,
...
void MeshRenderer::RebuildPipeline(const RenderTarget & renderTarget)
{
  if (m_pipeline)
  {
    auto * device = GetDrawTool().GetGPU().GetDevice();
  ...
  pipelineInfo.target_info = renderTarget.GetInfo().GetTargetsInfo();
